...
- Desktop `1025px+` should feel like one clean cover letter: one continuous text column, signature directly after the final line of copy, and the face-safe image grid on the right. The body text top aligns with the first image row top, and the signature bottom aligns with the last image row bottom. Do not split the About copy into two text columns.
